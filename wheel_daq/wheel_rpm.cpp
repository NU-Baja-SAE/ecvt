...
unsigned long last_rpm_calculation_time = 0;
...
void setup_wheel_rpm() {
    ...
    wheel_pulse_count = 0;
    last_rpm_calculation_time = millis();
    Serial.println("Wheel RPM sensor initialized");
}

float get_wheel_rpm() {
    unsigned long current_time = millis();
    unsigned long time_elapsed = current_time - last_rpm_calculation_time;

    if (time_elapsed >= RPM_CALCULATION_INTERVAL_MS) {
        float rpm = 0.0;

        if (time_elapsed > 0) {
            rpm = (wheel_pulse_count * 60000.0) / (time_elapsed * MAGNETS_PER_REVOLUTION);
        }
